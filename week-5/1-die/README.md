...
## Summary
Working off your previous `Die` class from [Die Class 1](../../week-4/6-die-class), implement a new `Die` class which takes an array of strings as its input.  When `Die#roll` is called, it randomly returns one of these strings.  If `Die.new` is passed an empty array, raise an `ArgumentError`.  It should work like this:
...
Use the *Well-Grounded Rubyist* as a reference as needed!
