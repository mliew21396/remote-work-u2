...
# created dependency off of the relative file path
class VirusPredictor

  def initialize(state_of_origin, population_density, population)
    # defines parameters for the class, first method that's run upon creation of class
...
#   private
  ...
  def virus_effects
    #default class that uses the two classes below it
    predicted_deaths
    speed_of_spread
  end
  ...
  def predicted_deaths
    ...
    #checks the population density for each state and estimates the number_of_deaths; then prints results to console
    if @population_density >= 200
#       number_of_deaths = (@population * 0.4).floor
      number_of_deaths = pop_flr(0.4)
    elsif @population_density >= 150
      number_of_deaths = pop_flr(0.3)
    elsif @population_density >= 100
      number_of_deaths = pop_flr(0.2)
    elsif @population_density >= 50
      number_of_deaths = pop_flr(0.1)
    else
      number_of_deaths = pop_flr(0.05)
    end
  ...
  def speed_of_spread #in months
    ...
    # checks the population density for given state and estimates the speed; returns the spread speed to the console
    speed = 0.0
...
STATE_DATA.each_key{|key|
    obj = VirusPredictor.new(key,STATE_DATA[key][:population_density],STATE_DATA[key][:population])
    obj.virus_effects
    }
# counter = 0
# while counter < STATE_DATA.length
#   obj = VirusPredictor.new(STATE_DATA,,)
# end
